...
type GetLinkifierMap = () => LinkifierMap;
...
export type MarkdownHelpers = {
    ...
    // linkifiers
    get_linkifier_map: () => LinkifierMap;
};
...
export function get_topic_links(topic: string, get_linkifier_map: GetLinkifierMap): TopicLink[] {
    ...
    for (const [pattern, {url_template, group_number_to_name}] of get_linkifier_map().entries()) {
        ...
        while ((match = pattern.exec(topic)) !== null) {
            ...
            let i = 0;
            const template_context = new Map<string, string>();
            while (i < matched_groups.length) {
                const matched_group = matched_groups[i];
                const current_group = i + 1;
                template_context.set(group_number_to_name[current_group], matched_group);
                i += 1;
            }
            const link_url = url_template.expand(Object.fromEntries(template_context));
...
function handleLinkifier({
    ...
    for (const match of matches) {
        const group_name = group_number_to_name[current_group];
        template_context[group_name] = match;
        current_group += 1;
    }
...
function handleTimestamp(time: string | number): string {
    let timeobject;
    let time_string;
    let time_number;
    if (typeof time === "string") {
        time_string = time;
        time_number = Number(time);
    } else {
    ...
    if (Number.isNaN(Number(time))) {
        ...
        timeobject = new Date(time_number * 1000);
    }
...
export type ParseOptions = {
    ...
    timestampHandler: (time: string | number) => string;
    gfm: boolean;
    tables: boolean;
    breaks: boolean;
    pedantic: boolean;
...
export function apply_markdown(message: {raw_content: string}): {
    raw_content: string;
    content: string;
    flags: string[];
...
export function get_topic_links_web(topic?: string): TopicLink[] {
    assert(web_app_helpers !== undefined);
    if (topic === undefined) {
        return [];
    }
    return get_topic_links(topic, web_app_helpers.get_linkifier_map);
}
