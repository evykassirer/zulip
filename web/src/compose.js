...
export let send_message = () => {
    ...
    if (request.type === "private") {
        ...
        request.to = emails;
        request.reply_to = recipient;
        request.private_message_recipient = recipient;
        request.to_user_ids = people.email_list_to_user_ids_string(emails);
        ...
        if (request.to_user_ids !== undefined) {
            request.to = people.user_ids_string_to_ids_array(request.to_user_ids);
        }
    } else {
        const topic = compose_state.topic();
        request.topic = util.is_topic_name_considered_empty(topic) ? "" : topic;
        const stream_id = compose_state.stream_id();
        request.stream_id = stream_id;
        request.to = stream_id;
    }
