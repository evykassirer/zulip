...
export let respond_to_message = (opts: {
    ...
    let topic = "";
    let pm_recipient_ids: number[] | undefined = [];
    if (msg_type === "stream") {
        ...
        pm_recipient_ids = [message.sender_id];
    } else {
        pm_recipient_ids = people.pm_with_user_ids(message);
    }
    ...
    compose_actions.start({
        ...
        stream_id,
        topic,
        ...(pm_recipient_ids !== undefined && {private_message_recipient_ids: pm_recipient_ids}),
