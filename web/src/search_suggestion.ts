...
// Interpret `operand` as a canonical `channel:` operand (a stream_id
// string), returning the channel it identifies, or undefined when the
// operand isn't a stream_id. `Filter.parse` leaves a typed channel
// name unresolved when it starts with a digit (e.g. "7th floor"), and
// `stream_data.get_sub_by_id_string` parses such a name as a number
// ("7th" -> stream 7), so require the operand to round-trip exactly
// before treating it as a stream_id.
function get_sub_for_channel_operand(operand: string): StreamSubscription | undefined {
    const sub = stream_data.get_sub_by_id_string(operand);
    if (sub?.stream_id.toString() === operand) {
        return sub;
    }
    return undefined;
}
...
function compute_multi_word_merged_term(
    ...
    if (prev.operator === "channel") {
        const sub = get_sub_for_channel_operand(prev.operand);
        if (sub !== undefined) {
...
function get_channel_suggestions(
    ...
    // will show up as a suggestion, which is a bit strange — and a
    // channel whose name is itself fully numeric can't be matched
    // by typing that name, since the digits are read as an id. We
    ...
    if (last.operator === "channel") {
        const sub = get_sub_for_channel_operand(last.operand);
        if (sub !== undefined) {
            query = sub.name;
        }
    }
...
function resolve_channel_term(
    ...
    if (get_sub_for_channel_operand(channel_term.operand) !== undefined) {
...
// 1. Completions that interpret the text after the space as the start
...
// 2. The first N "strong" multi-word matches (channels/topics whose
//    name starts with the full multi-word query). We only show max N
//    of these in tier 2 so that tier 3 always shows.
...
// MAX_STRONG_MULTI_WORD_MATCHES_ABOVE_FREE_TEXT is the N in step 2.
...
function get_suggestions_for_multi_word_channel_or_topic(
    ...
    if (prev.operator === "channel") {
        // Does the prev term resolve to a real channel? This gates tier 1
    ...
    // tiers 1 and 3 and the post-tier filterer loop. When prev
    ...
    if (prev_canonical !== undefined) {
        const tier_1_filterers = [
            get_operator_suggestions,
            get_is_filter_suggestions,
            ...(page_params.is_spectator ? [] : [get_sent_by_me_suggestions]),
            get_has_filter_suggestions,
        ];
        for (const filterer of tier_1_filterers) {
    ...
    // Tiers 2 / 4 / 5: multi-word matches.
    ...
    const tier_2 = strong.slice(0, MAX_STRONG_MULTI_WORD_MATCHES_ABOVE_FREE_TEXT);
    const tier_4 = strong.slice(MAX_STRONG_MULTI_WORD_MATCHES_ABOVE_FREE_TEXT);
    for (const suggestion of tier_2) {
    ...
    // resolved channel. This re-runs tier 1's operator-prefix
