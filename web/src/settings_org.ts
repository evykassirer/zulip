...
import {
    ...
    type SimpleDropdownRealmSettings,
    realm_setting_properties_schema,
    realm_user_settings_default_properties_schema,
    simple_dropdown_realm_settings_schema,
    stream_settings_properties_schema,
...
import {current_user, group_setting_type_schema, realm, realm_schema} from "./state_data";
...
const simple_dropdown_properties: (keyof SimpleDropdownRealmSettings)[] = [
...
function message_move_limit_setting_enabled(
    ...
    const setting_value_string = $(`#id_${CSS.escape(related_setting_name)}`).val();
    assert(typeof setting_value_string === "string");
...
function set_msg_move_limit_setting(property_name: MessageTimeLimitSetting): void {
...
export function set_message_retention_setting_dropdown(sub: StreamSubscription | undefined): void {
    ...
    if (sub !== undefined) {
        ...
        setting_value = z
            .number()
            .nullable()
            .parse(settings_components.get_stream_settings_property_value(property_name, sub));
    } else {
        property_name = "realm_message_retention_days";
        setting_value = z
            .number()
            .nullable()
            .parse(settings_components.get_realm_settings_property_value(property_name));
    }
...
export function discard_realm_property_element_changes(elem: HTMLElement): void {
    ...
    const property_value = settings_components.get_realm_settings_property_value(
        realm_setting_properties_schema.parse(property_name),
    );
    ...
    switch (property_name) {
        ...
        default:
            if (property_value !== undefined) {
                // TODO(evy) manually test all the settings before merging, just in case
                // there are other types that can end up here.
...
export function discard_stream_property_element_changes(
    ...
    const property_value = settings_components.get_stream_settings_property_value(
        stream_settings_properties_schema.parse(property_name),
        sub,
    );
    switch (property_name) {
...
export function discard_group_property_element_changes(elem: HTMLElement, group: UserGroup): void {
    ...
    if (property_name === "can_mention_group") {
        ...
        settings_components.set_group_setting_widget_value(
            pill_widget,
            group_setting_type_schema.parse(property_value),
        );
...
function set_up_dropdown_widget(
    ...
    const setting_dropdown_widget = new dropdown_widget.DropdownWidget({
        ...
        // TODO(evy): There are a lot of types that can be in the values of `realm`,
        // and I'm just assuming it's one of these three because of the type signature
        // of DropdownWidget, but maybe there's something better we can do here.
        default_id: z.union([z.string(), z.number()]).optional().parse(realm[setting_name]),
        unique_id_type,
...
export function build_page(): void {
    ...
    for (const property_name of simple_dropdown_properties) {
        settings_components.set_property_dropdown_value(
            simple_dropdown_realm_settings_schema.keyof().parse(property_name),
        );
    }
    ...
    $<HTMLSelectOneElement>("select:not([multiple])#id_realm_jitsi_server_url").on(
        "change",
        function (this: HTMLSelectOneElement, _event: JQuery.Event) {
    ...
    $("#id_realm_allow_message_editing").on("change", function (this: HTMLElement) {
        const is_checked = Boolean($(this).prop("checked"));
    ...
    $("#org-moving-msgs").on(
        ...
        function (this: HTMLElement) {
            ...
            const property_name = z
                .union([
                    z.literal("realm_edit_topic_policy"),
                    z.literal("realm_move_messages_between_streams_policy"),
                ])
