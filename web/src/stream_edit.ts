...
function stream_notification_reset(this: HTMLElement): void {
    ...
    stream_settings_api.bulk_set_stream_property(
        data,
        $(this).closest(".subsection-parent").find(".alert-notification"),
    );
}

function stream_setting_changed(this: HTMLInputElement): void {
    const sub = get_sub_for_target(this);
    const $status_element = $(this).closest(".subsection-parent").find(".alert-notification");
    const setting = settings_labels_schema.parse(this.name);
    const notification_setting = notification_labels_schema.safeParse(setting);
    ...
    stream_settings_api.set_stream_property(
        sub,
        {property: setting, value: this.checked},
        $status_element,
    );
}
