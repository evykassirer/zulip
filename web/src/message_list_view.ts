...
export type AllVisibilityPolicies = typeof user_topics.all_visibility_policies;
...
function populate_group_from_message(opts: {
    message: Message;
    date_unchanged: boolean;
    stream_url?: string;
    topic_url?: string;
    ...
    if (is_stream) {
        ...
        return {
            ...
            ...opts.subscription_markers,
            date: get_group_display_date(message),
            display_recipient,
            date_unchanged: opts.date_unchanged,
            topic_links,
            topic,
            recipient_bar_color: stream_color.get_recipient_bar_color(color),
            stream_privacy_icon_color: stream_color.get_stream_privacy_icon_color(color),
            ...
            match_topic: util.get_match_topic(message),
            stream_url: opts.stream_url,
            topic_url: opts.topic_url,
            stream_id,
            is_subscribed: stream_data.is_subscribed(stream_id),
...
export class MessageListView {
    ...
    _get_msg_edited_vars(message: Message): {
        last_edit_timestr: string | undefined;
        edited_in_left_col?: boolean;
        edited_alongside_sender?: boolean;
        edited_status_msg?: boolean;
    ...
    get_calculated_message_container_variables(
        ...
        */
        ...
        const variables = {
            ...
            ...this._get_msg_edited_vars(message),
        };
        return variables;
    }
    ...
    maybe_add_subscription_marker_to_group(
        group: MessageGroup,
        last_msg: Message | undefined,
        first_msg: Message,
    ): void {
        const markers = this.get_possible_group_subscription_markers(last_msg, first_msg);
        if (markers) {
    ...
    get_possible_group_subscription_markers(
        ...
        const last_subscribed = !last_msg.historical;
        const first_subscribed = !first_msg.historical;
        assert(first_msg.type === "stream");
        const stream_id = first_msg.stream_id;
    ...
    build_message_groups(messages: Message[]): MessageGroup[] {
        let group_message_containers: MessageContainer[] = [];

        const new_message_groups: MessageGroup[] = [];
        let current_group: MessageGroup;

        let prev: MessageContainer | undefined;

        const add_message_container_to_group = (message_container: MessageContainer): void => {
            group_message_containers.push(message_container);
        };

        const finish_group = (
            prev_message: Message | undefined,
            message_for_next_group: Message | undefined,
        ): void => {
            if (group_message_containers.length > 0) {
                current_group.message_containers = group_message_containers;
                new_message_groups.push(current_group);
            }
            group_message_containers = [];

            if (message_for_next_group === undefined) {
                return; // No more groups to create
            }
            current_group = populate_group_from_message({
                message: message_for_next_group,
                date_unchanged: same_day(message_for_next_group, prev_message),
                subscription_markers: this.get_possible_group_subscription_markers(
                    prev_message,
        ...
        for (const message of messages) {
            ...
            if (
                prev &&
                util.same_recipient(prev.msg, message) &&
                this.collapse_messages &&
                prev.msg.historical === message.historical
            ) {
            ...
            if (
                ...
                prev &&
                !prev.status_message &&
                same_day(prev.msg, message) &&
                prev.msg.sender_id === message.sender_id
            ) {
            ...
            const message_container: MessageContainer = {
                ...
                date_divider_html,
                mention_classname: null,
                ...this.get_calculated_message_container_variables(message, include_sender),
            };
        ...
        finish_group(undefined, undefined);
    ...
    merge_message_groups(new_message_groups: MessageGroup[], where: string): MessageActions {
    ...
    _rerender_header(message_containers: MessageContainer[]): void {
        ...
        // TODO(evy) figure out what to do here or if i can delete this
        // populate_group_from_message({
        //     message: group.message_containers[0]!.msg,
        // });
    ...
    rerender_messages(messages: Message[], message_content_edited = false): void {
        ...
        const message_containers: MessageContainer[] = messages
            .map((message) => this.message_containers.get(message.id))
            .filter(
