...
function restore_draft(draft_id: string): void {
    ...
    if (compose_args.type === "stream") {
        ...
        if (compose_args.private_message_recipient_ids.length > 0) {
            const user_emails = compose_args.private_message_recipient_ids
                .map((user_id) => people.get_by_user_id(user_id).email)
                .join(",");
            message_view.show([{operator: "dm", operand: user_emails}], {
                trigger: "restore draft",
            });
        }
    }
