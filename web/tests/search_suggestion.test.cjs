...
test("sent_by_me_suggestions", ({override}) => {
    ...
    // tier 1 surfaces above the literal-input row.
...
test("channel_multi_word_completion", ({override}) => {
    ...
    // typed query) ranks above tier 3, weak match ("Q1 automated
    // testing" matches phrase_match but doesn't start with the query)
    // ranks below.
    ...
    // Trailing fragment is a prefix of `has:*`, so tier 1 surfaces
    // those completions above the literal-input row. Only fires
    // because the single-word channel `automated` resolves.
    ...
    // `se` is an operator-prefix fragment: tier 1 surfaces both the
...
test("channel_multi_word_digit_prefix_completion", ({override}) => {
    ...
    // "7th floor" starts with digits, so `Filter.parse` leaves the
    // typed name unresolved; it must not be read as the stream_id of
    // the unrelated channel with id 7.
    ...
    let expected = ["channel:7th", "channel:8"];
    assert.deepEqual(suggestions, expected);
...
test("topic_multi_word_completion", ({override}) => {
    ...
    // Trailing fragment is a prefix of `has:*`, so tier 1 surfaces
    // those completions above the literal-input row.
